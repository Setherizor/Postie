...
import { setValue, fromBase64, getValue } from './helpers.js'
...
app.use(async function (request, response, next) {
  ...
  if (request.path == '/' && Boolean(p.state) && Boolean(p.code)) {
    ...
    try {
      ...
      await bot.db.read()
      bot.db.data.authTokens[p.state] = access_object
      bot.db.data.authTokens[p.state].timestamp = Date.now()
      await bot.db.write()
    ...
    response.cookie('authState', p.state, {
      maxAge: bot.db.data.authTokens[p.state].expires_in * 1000,
      httpOnly: true
    })
  ...
  if (cookie) {
    ...
    await bot.db.read()
    var token = getValue(bot.db.data, `authTokens.${cookie}.access_token`)
    if (!token) {
  ...
  if (request.path == '/logout' && request.access_token) {
    ...
    if (bot.db.data.authTokens[cookie]) {
      oauth
        .revokeToken(bot.db.data.authTokens[cookie].access_token, credentials)
        .then(debug)
...
async function checkTokenExpiry () {
  ...
  expiringTokenKeys.forEach(async key => {
    ...
    await bot.db.read()
    bot.db.data.authTokens[key] = await oauth.tokenRequest({
      refreshToken: bot.db.data.authTokens[key].refresh_token,
      grantType: 'refresh_token'
    })
    bot.db.data.authTokens[key].timestamp = Date.now()
    await bot.db.write()
  })
}
...
app.get('/botmasters/:guildId', async function (request, response) {
  ...
  try {
    if (guildId) {
      await bot.db.read()
      var masters = getValue(bot.db.data, `guilds.${guildId}.botMasters`)
...
app.post('/botmasters', async function (request, response) {
  ...
  try {
    if (guildId) {
      ...
      setValue(bot.db.data, `guilds.${guildId}.botMasters`, masters)
      await bot.db.write()
